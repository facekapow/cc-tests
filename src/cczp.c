...
void cczp_init(cczp_t zp) {
	cczp_t_AS_zp(zp)->mod_prime = &cczp_mod;
	const cc_unit* prime = cczp_prime((cczp_const_short_t)zp);
	cc_size n = ccn_n(cczp_n((cczp_const_short_t)zp), prime);
...
void cczp_mod(cczp_const_t zp, cc_unit* result, const cc_unit* operand, cc_ws_t ws) {
	...
	cc_size n = cczp_n((cczp_const_short_t)zp);

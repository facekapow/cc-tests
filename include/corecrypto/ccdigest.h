#ifndef CC_CCDIGEST_H
#define CC_CCDIGEST_H

#include <corecrypto/cc.h>
#include <corecrypto/ccn.h>

struct CC_ALIGNED(8) ccdigest_ctx {
	union {
		uint8_t u8;
		uint32_t u32;
		uint64_t u64;
...
#define ccdigest_ctx_t_AS_hdr(x) ((struct ccdigest_ctx*)x)

struct CC_ALIGNED(8) ccdigest_state {
	union {
		uint8_t u8;
		uint32_t u32;
		uint64_t u64;
...
#define ccdigest_state_t_AS_hdr(x) ((struct ccdigest_state*)x)
#define ccdigest_state_t_AS__ctx(x) ((struct ccdigest_ctx*)x)
#define ccdigest_state_t_AS__ctxt(x) ((ccdigest_ctx_t)x)
...
#define ccdigest_state(_di_, _ctx_)      ((struct ccdigest_state *)(&ccdigest_ctx_t_AS_hdr((ccdigest_ctx_t)(_ctx_))->state.u8 + sizeof(uint64_t)))
...
#define ccdigest_nbits(_di_, _ctx_)      (((uint64_t *)(&ccdigest_ctx_t_AS_hdr((ccdigest_ctx_t)(_ctx_))->state.u8))[0])
#define ccdigest_data(_di_, _ctx_)       (&ccdigest_ctx_t_AS_hdr((ccdigest_ctx_t)(_ctx_))->state.u8 + (_di_)->state_size + sizeof(uint64_t))
#define ccdigest_num(_di_, _ctx_)        (((unsigned int *)(&ccdigest_ctx_t_AS_hdr((ccdigest_ctx_t)(_ctx_))->state.u8 + (_di_)->state_size + sizeof(uint64_t) + (_di_)->block_size))[0])

#define ccdigest_u8(_state_)             (&ccdigest_state_t_AS_hdr((ccdigest_state_t)(_state_))->state.u8)
#define ccdigest_u32(_state_)            (&ccdigest_state_t_AS_hdr((ccdigest_state_t)(_state_))->state.u32)
#define ccdigest_u64(_state_)            (&ccdigest_state_t_AS_hdr((ccdigest_state_t)(_state_))->state.u64)
#define ccdigest_ccn(_state_)            (&ccdigest_state_t_AS_hdr((ccdigest_state_t)(_state_))->state.ccn)
